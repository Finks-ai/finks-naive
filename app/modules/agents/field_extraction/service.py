...
class FieldExtractionService:
    ...
    def _get_category_info(self, field: str) -> dict[str, Any]:
        ...
        return {"category": "Unknown", "type": "single", "is_multiselect": False}
        ...
        for category, category_fields in fields_by_category.items():
            ...
            if category_info["is_multiselect"]:
                ...
                if len(category_fields) > 1:
                    logger.debug(
                        f"Category {category} is single select, keeping only {category_fields[0]} out of {category_fields}"
                    )
    ...
    async def extract_fields(self, request: FieldExtractionRequest) -> FieldExtractionResponse:
        ...
        prompt = USER_PROMPT_TEMPLATE.format(query=request.query, available_fields=", ".join(request.available_fields))

        # Run the agent
        if not self.ai_agent:
            raise ValueError("Field extraction agent not initialized")
        ...
        validated_fields = [
            field
            for field in result.data.relevant_fields  # type: ignore[attr-defined]
        ...
        return FieldExtractionResponse(
            ...
            confidence=result.data.confidence,  # type: ignore[attr-defined]
    ...
    async def extract_fields_from_query(self, query: str) -> FieldExtractionResponse:
        ...
        request = FieldExtractionRequest(query=query, available_fields=self.available_fields)
...
async def extract_fields_step(context: dict[str, Any]) -> FieldExtractionResponse:
    ...
    if context.get("use_cache", False):
        try:
            ...
            async def cached_extract() -> FieldExtractionResponse:
