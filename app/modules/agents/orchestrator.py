...
import asyncio
from typing import Dict, Any, List, Callable, Optional, TypeVar, Tuple, TYPE_CHECKING
...
class AgentOrchestrator:
    ...
    async def _execute_group(
        ...
        logger.debug(f"Executing group with {len(parallel_steps)} parallel, "
        ...
        if parallel_steps:
            parallel_results = await self._execute_parallel(
                parallel_steps, context, previous_results
            )
    ...
    async def _execute_parallel(
        ...
        for step in steps:
            task = asyncio.create_task(
                self._execute_step(step, context, previous_results)
            )
        ...
        for i, (name, _) in enumerate(tasks):
            ...
            if isinstance(task_result, StepResult):
                ...
                logger.error(f"Parallel step {name} failed: {str(task_result)}")
    ...
    def _group_steps_by_dependencies(
        ...
        while remaining:
            ...
            if not ready:
                ...
                raise ValueError(
                    f"Cannot resolve dependencies. Remaining steps: "
    ...
    def _requires_context(self, func: Callable) -> bool:
        """Check if function requires context parameter."""
        import inspect
        sig = inspect.signature(func)
        return len(sig.parameters) >= 1
    
    def _requires_results(self, func: Callable) -> bool:
...
def sequential(*funcs: Callable) -> List[PipelineStep]:
...
def parallel(*funcs: Callable) -> List[PipelineStep]:
