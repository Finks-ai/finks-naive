...
class AgentRegistryBuilder:
    ...
    def __init__(self, registry: Optional["AgentRegistry"] = None) -> None:
        ...
        self._current_agent_type: AgentType | None = None
        self._current_info: dict[str, Any] = {}
        self._current_dependencies: list[AgentType] = []
    ...
    def _finalize_current_agent(self) -> None:
        """Finalize and register the current agent if any."""
        if self._current_agent_type and all(
            key in self._current_info
            for key in ["name", "description", "service_instance", "input_model", "output_model"]
        ):
...
class AgentRegistry:
    ...
    _available_fields: list[str] | None = None
    _field_instructions: dict[str, str] | None = None
    _field_categories: dict[str, Any] | None = None
    _unavailable_fields: list[str] | None = None
    _categorical_values: dict[str, dict[str, Any]] | None = None

    # AI Agent instances (created once per container)
    _ai_agents: ClassVar[dict[AgentType, Agent]] = {}
    ...
    def get(self, agent_type: AgentType) -> AgentInfo | None:
        """Get agent info by type."""
        return self._agents.get(agent_type)

    def get_service(self, agent_type: AgentType) -> Any | None:
    ...
    def list_agents(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            agent_type.value: {
                ...
                "dependencies": [dep.value for dep in info.dependencies] if info.dependencies else [],
            }
    ...
    def validate_dependencies(self, agent_type: AgentType) -> bool:
        ...
        for dependency in agent_info.dependencies or []:
    ...
    def _initialize_shared_configs(self) -> None:
        ...
        try:
            ...
            unavailable_fields_set = set(AgentRegistry._unavailable_fields or [])
            ...
            AgentRegistry._available_fields = sorted(all_instruction_fields - unavailable_fields_set)
            ...
            logger.info(
                f"Loaded shared configs: {len(AgentRegistry._available_fields or [])} available fields, "
                f"{len(AgentRegistry._field_instructions or {})} instructions, "
                f"{len(AgentRegistry._unavailable_fields or [])} unavailable fields, "
                f"{len(AgentRegistry._categorical_values or {})} categorical fields"
            )
        except Exception as e:
    ...
    @classmethod
    def get_ai_agent(cls, agent_type: AgentType, create_if_missing: bool = True) -> Agent | None:
    ...
    def _create_ai_agent(cls, agent_type: AgentType) -> None:
        ...
        try:
            if agent_type == AgentType.FIELD_EXTRACTION:
                ...
                from .field_extraction.prompts import SYSTEM_PROMPT

                cls._ai_agents[agent_type] = Agent(
                    model=settings.GEMINI_MODEL,
                    output_type=FieldExtractionResponse,  # type: ignore[arg-type]
                ...
                cls._ai_agents[agent_type] = Agent(
                    model=settings.GEMINI_MODEL,
                    output_type=InstructionProcessingResponse,  # type: ignore[arg-type]
                    system_prompt=SYSTEM_PROMPT.format(guidelines=GUIDELINES),
                )
                ...
                cls._ai_agents[agent_type] = Agent(
                    model=settings.GEMINI_MODEL,
                    output_type=SortingIntentResponse,  # type: ignore[arg-type]
                    system_prompt=SYSTEM_PROMPT.format(guidelines=GUIDELINES),
                )
                ...
                cls._ai_agents[agent_type] = Agent(
                    model=settings.GEMINI_MODEL,
                    output_type=SynthesisResponse,  # type: ignore[arg-type]
                    system_prompt=SYSTEM_PROMPT.format(guidelines=GUIDELINES, us_exchanges=", ".join(US_EXCHANGES)),
                )
                ...
                cls._ai_agents[agent_type] = Agent(
                    model=settings.GEMINI_MODEL,
                    output_type=QueryGenerationResponse,  # type: ignore[arg-type]
                    system_prompt=SYSTEM_PROMPT.format(guidelines=GUIDELINES),
                )
...
def get_agent(agent_type: AgentType) -> Any | None:
