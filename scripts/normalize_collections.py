...
# Setup logging BEFORE importing pymongo
...
class CollectionNormalizer:
        ...
        try:
            ...
            for collection_name, fields in collection_fields.items():
                ...
                cursor = collection.find(
                    {"symbol": {"$in": symbols}},
                    projection
                ).hint('symbol_1')  # Use index hint for performance
    ...
    async def normalize_all_collections(self, batch_size: int = 500, max_symbols: int = None):
            ...
            for i in range(0, len(symbols), batch_size):
                ...
                if batch_docs:
                    try:
                        ...
                        pbar.set_postfix(inserted=total_inserted, batch=i//batch_size + 1, pct=f"{(total_inserted/total_processed)*100:.1f}%")
    ...
    def _add_growth_metrics_to_batch(self, normalized_docs: List[Dict[str, Any]]):
        ...
        for doc in cursor:
            ...
            if period in ['Q1', 'Q2', 'Q3', 'Q4']:
                ...
                if (not income_data[symbol]['latest_quarterly'] or 
                    year > income_data[symbol]['latest_quarterly'].get('fiscal_year', 0)):
                ...
                # Track latest annual
                if (not income_data[symbol]['latest_annual'] or 
                    year > income_data[symbol]['latest_annual'].get('fiscal_year', 0)):
        ...
        for normalized_doc in normalized_docs:
            ...
            # Initialize metadata
            normalized_doc['growth_metrics_updated_at'] = datetime.now(timezone.utc)
            ...
            if latest_quarterly:
                ...
                if prior_year_quarter:
                    ...
                    normalized_doc['year_over_year_quarterly_revenue_growth'] = self._calculate_growth_rate(
                        prior_year_quarter.get('total_revenue'),
                        latest_quarterly.get('total_revenue')
                    )
                    
                    # EPS growth
                    current_eps = latest_quarterly.get('earnings_per_share_diluted') or latest_quarterly.get('earnings_per_share_basic')
                    prior_eps = prior_year_quarter.get('earnings_per_share_diluted') or prior_year_quarter.get('earnings_per_share_basic')
                    normalized_doc['year_over_year_quarterly_eps_growth'] = self._calculate_growth_rate(prior_eps, current_eps)
            ...
            if latest_annual:
                ...
                if prior_annual:
                    ...
                    normalized_doc['1year_annual_revenue_growth'] = self._calculate_growth_rate(
                        prior_annual.get('total_revenue'),
                        latest_annual.get('total_revenue')
                    )
                    
                    # 1-year EPS growth
                    current_eps = latest_annual.get('earnings_per_share_diluted') or latest_annual.get('earnings_per_share_basic')
                    prior_eps = prior_annual.get('earnings_per_share_diluted') or prior_annual.get('earnings_per_share_basic')
                ...
                if five_year_prior:
                    ...
                    normalized_doc['5year_annual_total_revenue_growth'] = self._calculate_cagr(
                        five_year_prior.get('total_revenue'),
                        latest_annual.get('total_revenue'),
                        5
                    )
                    
                    # 5-year EPS CAGR
                    beginning_eps = five_year_prior.get('earnings_per_share_diluted') or five_year_prior.get('earnings_per_share_basic')
                    ending_eps = latest_annual.get('earnings_per_share_diluted') or latest_annual.get('earnings_per_share_basic')
...
async def main():
    ...
    try:
        ...
        total_inserted = await normalizer.normalize_all_collections(batch_size=batch_size, max_symbols=max_symbols)
