...
class QueryPatternCache:
    ...
    def __init__(self) -> None:
        ...
        self._cache_collection: Any = None
        self._cache_stats = {"hits": 0, "misses": 0, "memory_hits": 0, "mongodb_hits": 0}
        self._normalization_rules = self._load_normalization_rules()

    def _load_normalization_rules(self) -> dict[str, list[Any]]:
        """Load query normalization rules from YAML file."""
        try:
            config_path = os.path.join(os.path.dirname(__file__), "query_normalization.yaml")
            with open(config_path) as f:
                ...
                if not isinstance(loaded_rules, dict):
                    raise ValueError("Expected dict from YAML file")
                rules: dict[str, list[Any]] = loaded_rules
                return rules
    ...
    def cache_collection(self) -> Any:
        ...
        if self._cache_collection is None:
            try:
                ...
                self._cache_collection.create_index("expires_at", expireAfterSeconds=0, background=True)
    ...
    def get_query_fingerprint(self, query: str, context: dict[str, Any] | None = None) -> str:
        """
        ...
        # Generate hash
        return hashlib.md5(json.dumps(cache_key_data, sort_keys=True).encode(), usedforsecurity=False).hexdigest()

    async def get_cached_result(self, cache_key: str, cache_levels: list[str] | None = None) -> dict[str, Any] | None:
    ...
    async def set_cached_result(
        self, cache_key: str, result: dict[str, Any], ttl_seconds: int = 3600, cache_levels: list[str] | None = None
    ) -> None:
    ...
    def _set_in_memory(self, key: str, data: dict[str, Any], ttl_seconds: int) -> None:
        ...
        self._memory_cache[key] = {"data": data, "expires": datetime.now() + timedelta(seconds=ttl_seconds)}
    ...
    def _cleanup_memory_cache(self) -> None:
        ...
        now = datetime.now()
        expired_keys = [k for k, v in self._memory_cache.items() if now >= v["expires"]]
        for key in expired_keys:
            del self._memory_cache[key]

    async def _get_from_mongodb(self, key: str) -> dict[str, Any] | None:
        """Get from MongoDB cache."""
        try:
            # Run in thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            doc = await loop.run_in_executor(
                None, lambda: self.cache_collection.find_one({"_id": key, "expires_at": {"$gt": datetime.utcnow()}})
            )
            if doc:
    ...
    async def _set_in_mongodb(self, key: str, data: dict[str, Any], ttl_seconds: int) -> None:
        ...
        try:
            ...
            await loop.run_in_executor(
                ...
                lambda: self.cache_collection.replace_one(
                    {"_id": key},
                    {"_id": key, "data": data, "expires_at": expires_at, "created_at": datetime.utcnow()},
                    upsert=True,
                ),
            )
        except Exception as e:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {**self._cache_stats, "hit_rate": f"{hit_rate:.2%}", "memory_size": len(self._memory_cache)}


# Global cache instance
_query_cache: QueryPatternCache | None = None
...
def cached_query(ttl_seconds: int = 3600) -> Callable[[Callable[..., Awaitable[T]]], Callable[..., Awaitable[T]]]:
    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            # Generate cache key
            cache_key = cache.get_query_fingerprint(request.query, {"max_results": getattr(request, "max_results", 10)})
            ...
            if result and hasattr(result, "dict"):
                await cache.set_cached_result(cache_key, result.dict(), ttl_seconds=ttl_seconds)
    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            if cached_fields:
                ...
                return cached_fields  # type: ignore[return-value]
            ...
            # Cache result
            if result:
                # Convert result to dict if it has a dict method (Pydantic model)
                if isinstance(result, dict):
                    cache_data = result
                elif hasattr(result, "dict") and callable(result.dict):
                    cache_data = result.dict()
                else:
                    # Skip caching if result can't be serialized
                    return result
