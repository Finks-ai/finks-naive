...
def fetch_field_values() -> dict[str, list[Any]]:
    ...
    fields = [field for field in sample_doc if field != "_id"]
...
def analyze_field_types(field_values: dict[str, list[Any]]) -> dict[str, dict[str, Any]]:
    """Analyze field types and characteristics."""
    field_analysis = {}

    for field, values in field_values.items():
        analysis = {"count": len(values), "type": "unknown", "sample_values": []}
...
def generate_search_space_configs(settings_dir: Path) -> bool:
    ...
    try:
        ...
        with open(analysis_output, "w") as f:
            ...
            f.write("# Search space field analysis\n")
        ...
        for field, analysis in field_analysis.items():
            if (analysis["type"] == "string" and analysis.get("subtype") == "categorical") or analysis[
                "type"
            ] == "boolean":
                categorical_fields[field] = {"values": analysis["values"], "count": analysis["count"]}

        with open(categorical_output, "w") as f:
            # Write timestamp comment
            f.write("# Categorical field values for search space\n")
            f.write(f"# updated_at: {datetime.utcnow().isoformat()}Z\n\n")
        ...
        for _, analysis in field_analysis.items():
