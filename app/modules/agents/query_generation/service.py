...
from app.core.constants import US_EXCHANGE_FILTER, US_EXCHANGES, VALUATION_RATIO_FIELDS
from app.core.retry import retry_on_ai_errors

from ..registry import AgentType, agent_registry
from ..synthesis.models import FieldPriority
from .models import QueryGenerationRequest, QueryGenerationResponse
...
class QueryGenerationService:
    ...
    def _ensure_exchange_filter(self, mongodb_query: dict[str, Any], request: QueryGenerationRequest) -> dict[str, Any]:
        ...
        if not has_exchange_mention and "exchange_acronym" not in str(mongodb_query):
            ...
            elif any(key.startswith("$") for key in mongodb_query):
                mongodb_query = {"$and": [mongodb_query, US_EXCHANGE_FILTER]}
                ...
                if field in VALUATION_RATIO_FIELDS and isinstance(value, dict):
                    ...
                    if has_lt and not has_gt:
                        ...
                        if threshold is not None and threshold > 0:
                            logger.info(
                                f"Adding positive constraint for valuation field: {field} (threshold: {threshold})"
                            )
                            new_value = {"$gt": 0}
                    ...
                    modified_conditions[field] = [
                        process_conditions(cond) if isinstance(cond, dict) else cond for cond in value
                    ]
    ...
    async def generate_query(self, request: QueryGenerationRequest) -> QueryGenerationResponse:
        ...
        # Run the agent
        if not self.query_generation_agent:
            raise ValueError("Query generation agent not initialized")
        result = await self.query_generation_agent.run(prompt)

        # Parse the query if it's a string
        mongodb_query = result.data.mongodb_query  # type: ignore[attr-defined]
        if isinstance(mongodb_query, str):
            try:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid JSON in MongoDB query: {mongodb_query}") from json_err
        ...
        return QueryGenerationResponse(
            ...
            query_explanation=result.data.query_explanation,  # type: ignore[attr-defined]
            estimated_results=result.data.estimated_results,  # type: ignore[attr-defined]
        )
    ...
    def _format_categorical_values(self, relevant_fields: list[str] | None = None) -> str:
        ...
        for field, info in self.categorical_values.items():
            if isinstance(info, dict) and "values" in info:
                ...
                should_include = (
                    field in priority_categorical_fields
                    or (relevant_fields and field in relevant_fields)
                    or (not relevant_fields and len(values) <= 20)
                )
    ...
    @retry_on_ai_errors(max_retries=3)
    async def generate_refinement_query(
        self, request: QueryGenerationRequest, previous_context: Any
    ) -> QueryGenerationResponse:
        ...
        if isinstance(mongodb_query, str):
            try:
                ...
                try:
                    ...
                    # Don't return empty query on parse error - raise exception instead
                    raise ValueError(f"Invalid JSON in refinement MongoDB query: {mongodb_query}") from json_err
...
async def generate_query_step(context: dict[str, Any], results: dict[str, Any]) -> QueryGenerationResponse:
