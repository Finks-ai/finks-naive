...
class InstructionProcessingService:
    ...
    async def process_instructions(self, request: InstructionProcessingRequest) -> InstructionProcessingResponse:
        ...
        valid_fields = [
            field
            for field in request.relevant_fields
            if field in self.available_fields and field not in self.unavailable_fields
        ]
        ...
        relevant_categorical = {
            field: self.categorical_values.get(field, {}) for field in valid_fields if field in self.categorical_values
        }
        ...
        if not self.ai_agent:
            raise ValueError("Instruction processing agent not initialized")
        result = await self.ai_agent.run(prompt)

        return InstructionProcessingResponse(
            field_interpretations=result.data.field_interpretations,  # type: ignore[attr-defined]
            processing_notes=result.data.processing_notes,  # type: ignore[attr-defined]
        )
    ...
    def _format_categorical_values(self, categorical: dict[str, dict[str, Any]]) -> str:
    ...
    async def process_query_instructions(self, query: str, relevant_fields: list[str]) -> InstructionProcessingResponse:
        ...
        request = InstructionProcessingRequest(
            query=query, relevant_fields=relevant_fields, field_instructions=self.field_instructions
        )
...
async def process_instructions_step(context: dict[str, Any], results: dict[str, Any]) -> InstructionProcessingResponse:
    ...
    result = await instruction_processing_service.process_query_instructions(
        query, field_extraction_result.relevant_fields
    )
