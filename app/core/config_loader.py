...
def load_config(config_name: str, config_dir: Path | None = None) -> dict[str, Any]:
    """
    ...
    with open(yaml_path, encoding="utf-8") as f:
        ...
        if not isinstance(result, dict):
            raise ValueError(f"Expected dict from YAML file, got {type(result)}")
        return result


def save_config(data: dict[str, Any], config_name: str, config_dir: Path | None = None) -> Path:
    """
