...
from .models import (
    ConfigType, RefreshStatus, RefreshResult, RefreshRequest,
    RefreshResponse, ConfigFileInfo, SettingsInfo, ConfigContent
)
...
class SettingsService:
    ...
    async def _refresh_single_config(
        ...
        try:
            ...
            for dep in config_info["requires"]:
                dep_path = self.settings_dir / dep
            ...
            output_path = self.settings_dir / config_info["output"]
            ...
            return RefreshResult(
                config_type=config_type,
                status=RefreshStatus.FAILED,
                message=f"Exception during refresh",
                duration_ms=duration_ms,
                error=str(e)
            )
    ...
    def get_config(self, config_type: ConfigType) -> Optional[ConfigContent]:
