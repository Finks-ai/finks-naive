...
class AgentPipelineService:
    ...
    async def process_query(self, request: AgentPipelineRequest) -> AgentPipelineResponse:
        ...
        context = {
            'request': request,
            'use_cache': HAS_CACHE and not request.previous_context
        }
    ...
    async def _cache_result(self, cache_key: str, response: AgentPipelineResponse):
        """Cache the response."""
        try:
            from app.core.cache import get_query_cache
            cache = get_query_cache()
    ...
    def _build_response(
        ...
        query_context = PreviousQueryContext(
            ...
            sorting_intent=SortingIntentResponse(
            ...
            ) if sorting_intent.has_sorting else None
        )
    ...
    def _build_processing_chain(self, results: Dict[str, StepResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "synthesis": {
                "unified_interpretation": synthesis_result.unified_interpretation,
                "field_priorities": {
                    fp.field_name: fp.priority 
                    for fp in synthesis_result.field_priorities
                },
